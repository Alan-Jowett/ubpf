...
typedef struct _ubpf_inst_filter
{
    ...
    bool (*destination)(int64_t dest);    ///< Function to validate the destination register.
...
ubpf_is_valid_instruction(const struct ebpf_inst inst, char** errmsg)
{
    ...
    if (filter->source != NULL && !filter->source(inst.src)) {
        *errmsg = ubpf_error("Invalid source register %d for instruction %2X.", inst.src, inst.opcode);
        return false;
    } else if (inst.src != 0) {
        *errmsg = ubpf_error("Source register must be zero for instruction %2X.", inst.opcode);
    }

    // Validate the destination register.
    if (filter->destination != NULL && !filter->destination(inst.dst)) {
        *errmsg = ubpf_error("Invalid destination register %d for instruction %2X.", inst.dst, inst.opcode);
        return false;
    } else if (inst.dst != 0) {
        *errmsg = ubpf_error("Destination register must be zero for instruction %2X.", inst.opcode);
    }

    // Validate the offset.
    if (filter->offset != NULL && !filter->offset(inst.offset)) {
        *errmsg = ubpf_error("Invalid offset %d for instruction %2X.", inst.offset, inst.opcode);
        return false;
    } else if (inst.offset != 0) {
        *errmsg = ubpf_error("Offset must be zero for instruction %2X.", inst.opcode);
    }

    // Validate the immediate value. Zero is always valid.
    if (filter->immediate != NULL && !filter->immediate(inst.imm)) {
        *errmsg = ubpf_error("Invalid immediate value %d for instruction %2X.", inst.imm, inst.opcode);
        return false;
    } else if (inst.imm != 0) {
        *errmsg = ubpf_error("Immediate value must be zero for instruction %2X.", inst.opcode);
    }
